...
func main() {
	fmt.Println("Advent of Code 2016 - Day 07")
	tls := 0
	ssl := 0
	...
	fmt.Printf("Part 1: %d of the IPs support TLS\n", tls)
	fmt.Printf("Part 1: %d of the IPs support SSL\n", ssl)
}

func NewIPv7(raw string) *IPv7 {
	hypernets := make([]string, 0)

	parts := strings.Split(raw, "[")

	// first part is supernet
	supernets := []string{parts[0]}
	for i := 1; i < len(parts); i++ {
		// first part is hypernet, second is supernet
		splitParts := strings.Split(parts[i], "]")
		if len(splitParts) != 2 {
			panic("Invalid ip!")
		}
		hypernets = append(hypernets, splitParts[0])
		supernets = append(supernets, splitParts[1])
	}
	return &IPv7{
		supernets: supernets,
		hypernets: hypernets,
...
func (ip *IPv7) SSL() bool {
	abas := []string{}
	for _, supernet := range ip.supernets {
		abas = append(abas, getABAs(supernet)...)
	}
	for _, aba := range abas {
		for _, hypernet := range ip.hypernets {
			if isBAB(aba, hypernet) {
				return true
			}
		}
	}
...
func (ip *IPv7) String() (s string) {
	s = fmt.Sprintf("IP\n  Supernets:\n")
	for _, supernet := range ip.supernets {
		s += fmt.Sprintf("    * %s\n", supernet)
	}

	s += fmt.Sprintf("  Hypernets:\n")
	for _, hypernet := range ip.hypernets {
		s += fmt.Sprintf("    * %s\n", hypernet)
	}
	return
...
func getABAs(str string) (res []string) {
	for i := 0; i < 3; i++ {
		for j := i; j < len(str)-2; j += 3 {
			if str[j] == str[j+2] && str[j] != str[j+1] {
				res = append(res, str[j:j+3])
			}
		}
	}
	return res
...
func isBAB(aba, str string) bool {
	bab := fmt.Sprintf("%s%s%s", string(aba[1]), string(aba[0]), string(aba[1]))
	return strings.Contains(str, bab)
}

const testpuzzleInput = `wlmcvfnfjyytctu[ornmuojenqtnhbx]ztsljuxapzxyukrtrnb[vwyozabsxvhgfocvmvw]ycticvyyxubyacik[rnfjsgktvqfmdkcml]ywsfuibwwstugijcnkk`
const puzzleInput = `xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
