...
func main() {
	...
	tower := NewTower(puzzle)
	fmt.Println("Name of root program:", tower.Name)

	// manually resolved part 2 by tracking down the imbalance and printing the corresponding root node needed to balanced the disc
	tower.FindImbalance()
}

func NewTower(in string) *Node {
	nodes := map[string]*Node{}
	relations := map[string][]string{}

	respMatch := regexp.MustCompile(`^([a-z]+) \((\d+)\)( -> (.*))?$`)
	lines := strings.Split(in, "\n")
	for _, line := range lines {
		m := respMatch.FindStringSubmatch(line)
		if len(m) < 5 {
		...
		if rel != "" {
			relations[name] = strings.Split(rel, ", ")
		}
	}

	for parent, children := range relations {
		p, found := nodes[parent]
		if !found {
...
func (n *Node) FindImbalance() {
	for _, c := range n.Children {
		...
		if !c.IsBalanced() {
			fmt.Println(c.Name, "is not balanced")
			c.FindImbalance()
		}
	}
...
func (n *Node) IsBalanced() bool {
	...
	w := n.Children[0].RecursiveWeight()
	fmt.Println(n.Name, "->", n.Children[0].Name, "=", w)
	for i := 1; i < len(n.Children); i++ {
		fmt.Println(n.Name, "->", n.Children[i].Name, "=", n.Children[i].RecursiveWeight())
		if n.Children[i].RecursiveWeight() != w {
			res = false
		}
	}
	return res
...
func (n *Node) RecursiveWeight() int {
	weight := n.Weight
	for _, c := range n.Children {
		weight += c.RecursiveWeight()
	}
	return weight
}
...
func (n *Node) printSub(depth int) {
	printWithPrefix(depth, fmt.Sprintf("%s (%d | %d)", n.Name, n.Weight, n.RecursiveWeight()))
	for _, c := range n.Children {
		c.printSub(depth + 1)
	}
}

func printWithPrefix(depth int, s string) {
	for i := 0; i < depth; i++ {
		fmt.Print("\t")
	}
	fmt.Println(s)
...
const puzzle = `suvtxzq (242) -> tdoxrnb, oanxgk
smjsfux (7)
oanxgk (68)
tvvfszm (66)
