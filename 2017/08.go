...
func main() {
	...
	prog := NewProgram(puzzle)
	regs, largest := prog.Exec()

	fmt.Println("Largest register:", regs.Largest())
	fmt.Println("Largest register during execution:", largest)
}
...
func NewProgram(in string) Program {
	...
	opMatcher := regexp.MustCompile(`^([a-z]+) (inc|dec) (-?\d+) if ([a-z]+) (==|!=|<|<=|>|>=) (-?\d+)$`)
	lines := strings.Split(in, "\n")
	for _, line := range lines {
		m := opMatcher.FindStringSubmatch(line)
		if len(m) < 7 {
		...
		case "dec":
			op.op = DecOp
		default:
			panic("Unknown op: " + m[2])
		}

		var err error
		op.val, err = strconv.Atoi(m[3])
		if err != nil {
			panic(err)
		}
		...
		case "==":
			op.cond.comp = EqComp
		case "!=":
			op.cond.comp = NeqComp
		case "<":
			op.cond.comp = LtComp
		case "<=":
			op.cond.comp = LteqComp
		case ">":
			op.cond.comp = GtComp
		case ">=":
			op.cond.comp = GteqComp
		default:
			panic("Unknown comparison: " + m[5])
		}

		op.cond.val, err = strconv.Atoi(m[6])
		if err != nil {
			panic(err)
		}
...
func (op Operation) Exec(regs Regs) {
	if op.cond.IsTrue(regs) {
		switch op.op {
		case IncOp:
			regs[op.reg] = regs[op.reg] + op.val
		case DecOp:
			regs[op.reg] = regs[op.reg] - op.val
		default:
			panic("Unknown op")
...
func (c Condition) IsTrue(regs Regs) bool {
	switch c.comp {
	case EqComp:
		return regs[c.reg] == c.val
	case NeqComp:
		return regs[c.reg] != c.val
	case GtComp:
		return regs[c.reg] > c.val
...
const puzzle = `ba dec 37 if znx != 0
