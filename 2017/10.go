...
func main() {
	...
	fmt.Println("KnotHash value:", KnotHash(256, ReadLengths(puzzle)))
	fmt.Println("KnotHash hash: ", KnotHashBytes(ReadLengthsToBytes(puzzle)))
}

func ReadLengths(in string) []int {
...
func KnotHash(size int, lengths []int) int {
	...
	for _, length := range lengths {
		for step := 0; pos+step < pos+length-step-1; step++ {
			//pos <-> pos+length
			//pos+1 <-> pos+length-1
			elms[(pos+step)%size], elms[(pos+length-step-1)%size] = elms[(pos+length-step-1)%size], elms[(pos+step)%size]
		}
...
func ReadLengthsToBytes(in string) []byte {
	return append([]byte(in), 17, 31, 73, 47, 23)
}

func KnotHashBytes(lengths []byte) string {
	const rounds = 64
	const size = 256
	const blocks = 16
...
const puzzle = `206,63,255,131,65,80,238,157,254,24,133,2,16,0,1,3`
