...
func main() {
	...
	fmt.Println("Passphrases with no repeated words:", HasNoRepeatedWords(puzzle))
	fmt.Println("Passphrases with no repeated words or anagrams:", IsValidPassphrase(puzzle))
}

func IsValidPassphrase(in string) int {
	passes := strings.Split(in, "\n")
	valid := 0

	for _, pass := range passes {
		if HasNoRepeatedWord(pass) && HasNoAnagram(pass) {
			valid++
		}
	}
...
func HasNoRepeatedWords(in string) int {
	passes := strings.Split(in, "\n")
	valid := 0

	for _, pass := range passes {
		if HasNoRepeatedWord(pass) {
			valid++
		}
	}
...
func HasNoRepeatedWord(in string) bool {
...
func HasNoAnagram(in string) bool {
	words := strings.Fields(in)
	for i := 0; i < len(words); i++ {
...
const puzzle = `pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
