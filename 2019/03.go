...
func main() {
	...
	wiremoves := Load(puzzleInput)
	board := NewCircuitboard()
	_, _ = board.AddWire(1, wiremoves[0])
	dist, delay := board.AddWire(2, wiremoves[1])
	fmt.Println("Closest intersection:", dist)
	fmt.Println("Smallest delay:", delay)
}
...
func NewCircuitboard() *Circuitboard {
	return &Circuitboard{wires: map[Coord]Wire{}}
}

func (board *Circuitboard) AddWire(id int, wire []WireMove) (int, int) {
	mindist := 999999
	minsteps := 999999
	step := 0
	pos := Coord{0, 0}
	for _, m := range wire {
		...
		case "R":
			for i := 0; i < m.Dist; i++ {
				pos.X++
				step++
				if w, found := board.wires[pos]; found && w.ID != id {
					// crossed wires
					d := ManhattanDist(pos, Coord{0, 0})
					if d < mindist {
						mindist = d
					}
					s := step + w.Step
		...
		default:
			panic(fmt.Sprintf("unknown direction: %s", m.Direction))
		}
	}
...
func Load(in string) [][]WireMove {
	...
	for _, l := range strings.Split(in, "\n") {
		...
		for _, m := range strings.Split(l, ",") {
			...
			wire = append(wire, WireMove{string(m[0]), i})
		}
		out = append(out, wire)
	}
	return out
