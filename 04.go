...
func main() {
	fmt.Println("Advent of Code 2016 - Day 04")

	e := regexp.MustCompile(`(.*)-(\d+)\[([a-z]+)\]$`)
	sum := 0
	/*
		for i := 0; i < 50; i++ {
			str := ShiftString("qzmt-zixmtkozy-ivhz", i)
			fmt.Printf("%.3d %s\n", i, str)
		}
		fmt.Println()
	*/
	...
	for _, line := range strings.Split(puzzleInput, "\n") {
		fields := e.FindStringSubmatch(line)
		if len(fields) != 4 {
			panic("bad line: " + line)
		}
		...
		if checker.Verify(checksum) {
			sum += id
			decryptedRoom := ShiftString(room, id)
			if decryptedRoom[:5] == "north" {
				fmt.Printf("Part 2: North Pole objects are stored in room %d\n", id)
			}
		}
	}

	fmt.Println("Part 1: Room id sum:", sum)
}

func ShiftString(str string, rot int) string {
	const first = byte('a')
	const last = byte('z')
	const count = int(last - first + 1)

	// for some unknown reason this is needed for it to work..?
	rot = rot % count
	...
	for i := range str {
		...
		res += string(byte(int(str[i]-first+byte(rot))%count) + first)
	}
	return res
}
...
func (c *Checksum) Verify(sum string) bool {
	sort.Sort(c.characters)
	for i := range sum {
		if c.characters[i].char != sum[i] {
			return false
		}
	}
	return true
...
const puzzleInput = `fubrjhqlf-edvnhw-dftxlvlwlrq-803[wjvzd]
