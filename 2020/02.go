...
func ValidatePasswordFile(policy int, in string) int {
	...
	valid := 0
	for n, s := range strings.Split(in, "\n") {
		passwordEntry := re.FindStringSubmatch(s)
		if len(passwordEntry) < 5 {
			panic(fmt.Sprintf("Line %d did not match regexp", n))
		}
		min, err := strconv.Atoi(passwordEntry[1])
		if err != nil {
			panic(errors.Wrap(err, "failed to get min"))
		}
		max, err := strconv.Atoi(passwordEntry[2])
		if err != nil {
			panic(errors.Wrap(err, "failed to get max"))
		}
		seq := passwordEntry[3]
		pass := passwordEntry[4]

		if ok := validators[policy](min, max, seq, pass); !ok {
			continue
		}
		valid++
	}
...
func ValidatePassword(min, max int, seq string, pass string) bool {
	n := strings.Count(pass, seq)
	if n < min || n > max {
		return false
	}
	return true
...
func ValidatePassword2(min, max int, seq string, pass string) bool {
	if len(seq) != 1 {
