...
func main() {
	reName := regexp.MustCompile("(Sue [0-9]+):")
	reTrees := regexp.MustCompile("trees: ([0-9]+)")
	rePerfumes := regexp.MustCompile("perfumes: ([0-9]+)")
	reCats := regexp.MustCompile("cats: ([0-9]+)")
	rePomer := regexp.MustCompile("pomeranians: ([0-9]+)")
	reAki := regexp.MustCompile("akitas: ([0-9]+)")
	reCars := regexp.MustCompile("cars: ([0-9]+)")
	reGoldfish := regexp.MustCompile("goldfish: ([0-9]+)")
	reViz := regexp.MustCompile("vizslas: ([0-9]+)")
	reChild := regexp.MustCompile("children: ([0-9]+)")
	reSamo := regexp.MustCompile("samoyeds: ([0-9]+)")
	...
	for {
		...
		matches = reTrees.FindStringSubmatch(line)
		if len(matches) > 0 {
			trees, err := strconv.Atoi(matches[1])
			if err != nil {
				panic("fail")
			}
			if trees != 3 {
		...
		matches = rePerfumes.FindStringSubmatch(line)
		if len(matches) > 0 {
			perfumes, err := strconv.Atoi(matches[1])
			if err != nil {
				panic("fail")
			}
			if perfumes != 1 {
		...
		matches = reCars.FindStringSubmatch(line)
		if len(matches) > 0 {
			cars, err := strconv.Atoi(matches[1])
			if err != nil {
				panic("fail")
			}
			if cars != 2 {
		...
		matches = reGoldfish.FindStringSubmatch(line)
		if len(matches) > 0 {
			goldfish, err := strconv.Atoi(matches[1])
			if err != nil {
				panic("fail")
			}
			if goldfish != 5 {
		...
		matches = reViz.FindStringSubmatch(line)
		if len(matches) > 0 {
			viz, err := strconv.Atoi(matches[1])
			if err != nil {
				panic("fail")
			}
			if viz != 0 {
		...
		matches = reAki.FindStringSubmatch(line)
		if len(matches) > 0 {
			aki, err := strconv.Atoi(matches[1])
			if err != nil {
				panic("fail")
			}
			if aki != 0 {
		...
		matches = rePomer.FindStringSubmatch(line)
		if len(matches) > 0 {
			pomer, err := strconv.Atoi(matches[1])
			if err != nil {
				panic("fail")
			}
			if pomer != 3 {
		...
		matches = reSamo.FindStringSubmatch(line)
		if len(matches) > 0 {
			samo, err := strconv.Atoi(matches[1])
			if err != nil {
				panic("fail")
			}
			if samo != 2 {
		...
		matches = reCats.FindStringSubmatch(line)
		if len(matches) > 0 {
			cats, err := strconv.Atoi(matches[1])
			if err != nil {
				panic("fail")
			}
			if cats != 7 {
		...
		matches = reChild.FindStringSubmatch(line)
		if len(matches) > 0 {
			child, err := strconv.Atoi(matches[1])
			if err != nil {
				panic("fail")
			}
			if child != 3 {
