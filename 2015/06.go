...
func NewCommand(s string) Command {
	...
	case "toggle":
		c.cmd = TOGGLE
	default:
		panic("unknown command: " + submatches[1])
	}

	var err error
	
	if c.x1, err = strconv.Atoi(submatches[2]); err != nil {
		panic("atoi failed: "+err.Error())
	}
	if c.y1, err = strconv.Atoi(submatches[3]); err != nil {
		panic("atoi failed: "+err.Error())
	}
	if c.x2, err = strconv.Atoi(submatches[4]); err != nil {
		panic("atoi failed: "+err.Error())
	}
	if c.y2, err = strconv.Atoi(submatches[5]); err != nil {
		panic("atoi failed: "+err.Error())
	}
...
func (g *Grid) ProcessCommand(cmd Command) {
	var f func(int) int
...
func main() {
	...
	for {
		...
		grid.ProcessCommand(NewCommand(word))
	}
