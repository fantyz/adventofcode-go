...
func Day21() {
	...
	fmt.Println("The cheapest gear that still wins over the boss cost:", lowestToWin)
...
func OptimizeRPGSimulator20XXGear(boss Warrior) (int, int) {
	...
	lowestCostToWin := -1
	highestCostToLoose := -1
	for weapon := range weapons {
		for armor := range armors {
			for ring1 := range rings {
				for ring2 := range rings {
					if ring1 == ring2 {
						continue
					}
					...
					if RPGSimulator20XX(Warrior{Hitpoints: 100, Damage: dmg, Armor: arm}, boss) {
						...
						if highestCostToLoose < 0 || cost > highestCostToLoose {
							highestCostToLoose = cost
						}
					}
				}
			}
